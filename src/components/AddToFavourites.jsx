               ...
               state.properties.map((property)=>{
                return (
                 <div key={property.id} className="property-container favourite-container">
                        ...
                        <button onClick={()=>{handlePropertyRemove(property.id)}} className="delete-button">Delete<DeleteIcon style={{fontSize:'25px'}}/></button>
                        <Link to={`/property/${property.id}`}><button className="rental-button">See details<CallMadeIcon style={{fontSize:"20px", marginLeft:"15px", marginBottom:"-5px"}}/></button></Link>
                </div>
                )
               }) 
            )}
