...
export default function Details() {
  ...
  const {id} = useParams();
  const foundProperty = properties.find((property) => property.id ===  Number(id));

  return (
    <div className='product-container'>
      <div className='title-container'>
      <h2 className='home-title'>Property details: <span style={{color:"#f7eedd"}}>{foundProperty.name}</span></h2>
      </div>
